...
def start_download_threads():
    global _worker_started, download_threads
...
def game_menu(screen, font, active_mappings, json_file, save_dir):
    ...
    FONT_SIZE, LINE_HEIGHT = 28, 44  # 调整为合适尺寸
    LEFT_MARGIN, RIGHT_MARGIN = 100, 100
    TOP_MARGIN, BOTTOM_MARGIN = 70, 90
    HIGHLIGHT_PAD_Y, HIGHLIGHT_PAD_X = 3, 16
    BORDER_RADIUS = 20
    ...
    h_font = pygame.font.Font(font_path, 20) if os.path.exists(font_path) else pygame.font.SysFont("simhei", 20)  # 页脚字体
    ...
    while True:
        ...
        for i, item in enumerate(page_data):
            ...
            if i == sel_idx:
                rect_h = min(LINE_HEIGHT - 8, g_font.get_height() + 2 * HIGHLIGHT_PAD_Y)
                rect_w = sw - LEFT_MARGIN - RIGHT_MARGIN + 2 * HIGHLIGHT_PAD_X
                rect_x = LEFT_MARGIN - HIGHLIGHT_PAD_X
                rect_y = row_top + (LINE_HEIGHT - rect_h) // 2
