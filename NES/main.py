...
# --- UI 布局参数 (3列2行，每页6个) ---
SLOT_W, SLOT_H = 260, 160
COLS, ROWS = 3, 2
ITEMS_PER_PAGE = COLS * ROWS
HORIZONTAL_MARGIN = 80  # 左右边距
VERTICAL_MARGIN = 170   # 上下边距
H_GAP = (SW - 2 * HORIZONTAL_MARGIN - COLS * SLOT_W) // (COLS + 1)  # 列间距自适应
V_GAP = 70  # 行间距
COL_SPACING = HORIZONTAL_MARGIN + H_GAP + SLOT_W // 2
ROW_START_Y = VERTICAL_MARGIN + SLOT_H // 2
V_SPACING = SLOT_H + V_GAP

def create_slot_surface(img_path, slot_w, slot_h, scale_inner=0.55):
    ...
    if os.path.exists(img_path):
        try:
            ...
            max_w = int(slot_w * scale_inner)
            max_h = int(slot_h * scale_inner)
...
for config in GAME_CONFIGS:
    ...
    config["surf_n"] = create_slot_surface(img_path, SLOT_W, SLOT_H, 0.55)
    config["surf_s"] = create_slot_surface(img_path, SLOT_W, SLOT_H, 0.65)
...
while running:
    ...
    # 渲染九宫格游戏槽位 (3列2行，每个槽位单独分开)
    start_idx = current_page * ITEMS_PER_PAGE
    for i in range(ITEMS_PER_PAGE):
        idx = start_idx + i
        if idx >= len(GAME_CONFIGS): break
        r, c = i // COLS, i % COLS
        x = HORIZONTAL_MARGIN + H_GAP + c * (SLOT_W + H_GAP) + SLOT_W // 2
        y = ROW_START_Y + r * (SLOT_H + V_GAP)
        ...
        screen.blit(name_surf, (x - name_surf.get_width()//2, rect.bottom + 15))
    ...
    for event in pygame.event.get():
        ...
        # 导航控制逻辑
            ...
            if event.key == pygame.K_LEFT: selected_index = (selected_index - 1) % len(GAME_CONFIGS)
                ...
                if new_idx >= 0: selected_index = new_idx
                else: selected_index = ((selected_index % ITEMS_PER_PAGE) + (total_pages - 1) * ITEMS_PER_PAGE)
            elif event.key == pygame.K_DOWN:
                new_idx = selected_index + COLS
                if new_idx < len(GAME_CONFIGS): selected_index = new_idx
                else: selected_index = selected_index % COLS
        elif event.type == pygame.JOYHATMOTION:
            hx, hy = event.value
