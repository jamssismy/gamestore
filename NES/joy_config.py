...
_controller_db_loaded = False
_cached_input_configs = {}
_input_cfg_mtime = None
...
def load_controller_db():
    global _controller_db_loaded
    if _controller_db_loaded:
        return
    ...
    _controller_db_loaded = True

# 原始函数：解析ES输入配置
def load_active_mappings(force_reload=False):
    global _cached_input_configs, _input_cfg_mtime
    load_controller_db()  # 新增：在解析前先加载数据库

    cfg_path = "/userdata/system/configs/emulationstation/es_input.cfg"
    current_mtime = os.path.getmtime(cfg_path) if os.path.exists(cfg_path) else None

    if force_reload or _input_cfg_mtime != current_mtime:
        _cached_input_configs = {}
        _input_cfg_mtime = current_mtime
        if os.path.exists(cfg_path):
            try:
                ...
                for config in root.findall('inputConfig'):
                    ...
                    _cached_input_configs[dev_name] = {
            ...
            log_message(f"未找到输入配置文件: {cfg_path}")
    ...
    for i in range(joy_count):
        try:
            ...
            if raw_name in _cached_input_configs:
                active_maps[i] = _cached_input_configs[raw_name]
